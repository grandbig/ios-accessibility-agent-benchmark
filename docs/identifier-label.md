...
| ⑦ | ③ + 親に `accessibilityElement(children: .contain)` | ✅ 検出 | （親は `Other` 要素） | '子ボタン7' | **③の上書きが解消** |
...
## 公式の位置づけと回避策: accessibilityElement(children: .contain)

この「親の identifier が子を上書きする」挙動は、`accessibilityIdentifier(_:)` の公式 API リファレンスには
明記されていない（未文書の副作用）。一方で **Apple エンジニアが Apple Developer Forums で挙動を認め、
回避策を accepted answer として提示している**:

> SwiftUI: Adding accessibility identifiers to VStack — Apple Developer Forums
> https://developer.apple.com/forums/thread/671189

提示された回避策は、コンテナに `accessibilityElement(children: .contain)` を付けること。
これによりコンテナが独立したアクセシビリティ要素になり、子は自分の identifier を保持できる。
`accessibilityElement(children:)`（`.ignore` / `.combine` / `.contain`）自体は公式に文書化されている
...
HStack {
    ...
    Button("子ボタン7") {}
        .buttonStyle(.bordered)
        .accessibilityIdentifier("idlabel.case7.child")   // ← 子の id
}
.accessibilityElement(children: .contain)                 // ← これを足すと…
.accessibilityIdentifier("idlabel.case7.parent")          // ← 親の id はコンテナに残る
```

XCUITest からの見え方（実測）:

| クエリ | ③（.contain なし） | ⑦（.contain あり） |
| -- | :--: | :--: |
| `buttons["idlabel.case7.child"]`（子id） | ❌ 上書きで消える | ✅ **保持される** |
| `buttons["idlabel.case7.parent"]`（親id をButtonとして） | ✅ 当たってしまう | — |
| `otherElements["idlabel.case7.parent"]`（親をコンテナとして） | — | ✅ 独立した要素 |

→ `.contain` を付けると、**子は自分の identifier を保持し、親は独立した `Other` 要素として
自分の identifier を持つ**。これは UIKit で観測した構造（親＝コンテナ要素、子＝自分の id）と一致する。
つまり SwiftUI でも、明示的に grouping を宣言すれば UIKit と同じ「View 単位で独立した identifier」を得られる。
...
2. **親コンテナに安易に `accessibilityIdentifier` を付けない**。子へ伝播・上書きされ、
   子個別の identifier 検出を壊し、重複マッチも招く（②③）。コンテナにも id を付けたい場合は
   `accessibilityElement(children: .contain)` を併用すると、子の identifier を保持したまま
   コンテナに id を持たせられる（⑦・Apple 公式フォーラムの回避策）。
